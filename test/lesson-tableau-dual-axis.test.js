// Tests for tableau-dual-axis lesson: comparing measures on different scales.

import { test, describe } from "node:test";
import assert from "node:assert/strict";

import { lesson } from "../lib/guide/lessons/tableau-dual-axis.js";
import { validateLesson } from "../lib/guide/spec.js";

describe("lesson: tableau-dual-axis", () => {
  test("lesson structure is valid", () => {
    assert.equal(lesson.id, "tableau-dual-axis");
    ...
    for (const step of lesson.steps) {
      if (step.spotlightLabel) {
        assert.ok(
          ...
          `Step ${step.id}: spotlight label too long`,
        );
        assert.ok(
