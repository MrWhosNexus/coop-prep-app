...
/**
 * Grades below this count as a full lapse and reset the repetition count.
 * Note that GRADE.HARD (3) sits below this bar: every caller in this
 * codebase passes 3 to mean "shaky" -- an incorrect-but-near typed answer
 * (lib/games/scoring.js gradeFromMatch), a match solved only after several
 * tries (MatchGame), a flagged guess that happened to land (lib/exam/
 * review.js). When 3 was the pass bar, four consecutive "I was unsure"
 * grades walked a card out to a 23-day interval with lapses stuck at 0 --
 * being wrong scheduled the card FURTHER out. HARD now has its own branch
 * in reviewCard(): it shrinks-or-holds the interval instead of resetting
 * it, but it is never a pass.
 */
...
export function reviewCard(card, grade, opts = {}) {
  ...
  if (q === GRADE.HARD) {
    // HARD: the learner did NOT demonstrate reliable recall -- every caller
    // passes 3 for a near-wrong answer, a multi-try guess, or a flagged
    // guess that landed. The interval is HALVED (floored at 1) so the card
    // comes back sooner without the full relearn of AGAIN, ease
    // still drops via the SM-2 delta above, and reps is left alone so a
    // card can never graduate to "mature" on hardness. It counts as a
    // lapse: `lapses` is the one field that surfaces a chronically shaky
    // concept, and before this branch existed it sat at 0 across entire
    // decks while opposite-answer near-misses promoted cards. The exam
    // path's "right but flagged" also lands here; over-counting a lucky
    // guess as a lapse merely resurfaces it sooner, while under-counting
    // hides a failing concept forever -- the costs are not symmetric.
    reps = card.reps;
    lapses += 1;
    // 0.5, NOT 1.2. A multiplier above 1 still walks a MISSED card further
    // out on every miss -- 17 -> 20 -> 24 -> 29 days -- which is the exact
    // defect this branch exists to remove, merely slowed down. Verified by
    // runtime probe, not by the suite: the tests were green while the
    // interval still grew, because they asserted lapses and ease and never
    // asserted the direction the interval moved.
    intervalDays = Math.max(1, Math.round(card.intervalDays * 0.5));
  } else if (q < PASS_GRADE) {
...
/**
 * Once the deadline is behind us there is no date to clamp to, but spacing
 * must keep working — the old floor of 1 silently turned the scheduler into
 * "everything is due every day", which is the opposite of spaced repetition.
 * 28 days is the rolling window we fall back to: it is past the classic
 * 21-day "mature card" threshold (see deckStats), so cards can still
 * graduate, while keeping every concept in rotation at least monthly for a
 * learner who is now using the material on the job. The exact number is not
 * magic; what is load-bearing is that it is a real horizon, never 1.
 */
export const ROLLING_WINDOW_DAYS = 28;

/**
 * Cap intervals so nothing schedules past a hard deadline (e.g. the
 * program end). Returns the largest sensible maxIntervalDays. Before the
 * deadline that is the days remaining; once the deadline has passed the cap
 * switches to ROLLING_WINDOW_DAYS — the deadline no longer constrains
 * anything, and clamping to it (the old Math.max(1, ...) floor) destroyed
 * spacing by making every card due daily.
 * @param {Date|number|string} deadline
 * @param {Date|number|string} [now]
 * @returns {number} days
 */
export function horizonTo(deadline, now) {
  const remaining = daysBetween(dayISO(now), dayISO(deadline));
  return remaining >= 1 ? remaining : ROLLING_WINDOW_DAYS;
}
