...
export {
  ...
  daysUntilFellowship, fellowshipPhase, daysIntoFellowship,
  readinessScore, todayISO, defaultState,
...
import { markComplete, markLabComplete, saveQuizScore, todayISO } from "./progress.js";
...
/* Guided-lab completion goes through the same reward pipeline as lessons so
   XP toasts, level-ups, and achievements fire for lab work too. Idempotence
   lives in markLabComplete (labs[] ledger): a re-completed lab comes back as
   `final: null`, exactly like re-completing a reading. */
export function doCompleteLab(before, guidedId, score) {
  const after = markLabComplete(before, guidedId, score);
  if (after === before) return { final: null, queued: [], reward: NO_REWARD };
  return buildRewardAndFinal(before, after);
}
