...
export function specMatches(spec, expected) {
  // Fail LOUDLY on keys we do not understand. This used to silently skip
  // them, which let a lesson grade { dual: true } — a key no shelf or mark
  // ever had — and auto-pass the step: the learner "completed" a dual-axis
  // chart by doing nothing. An unknown key is an authoring bug, not a match.
  const recognized = new Set([...MULTI_SHELVES, ...SINGLE_SHELVES, "mark", "filters"]);
  const unknown = Object.keys(expected).filter((k) => !recognized.has(k));
  if (unknown.length > 0) {
    throw new Error(
      `specMatches: unrecognized key(s) in expected spec: ${unknown.join(", ")}. ` +
        `Recognized keys: ${[...recognized].join(", ")}.`
    );
  }
  const misses = [];
