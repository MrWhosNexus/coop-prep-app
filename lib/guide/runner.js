...
//     show-me → 0.3
//   passed step   → gradeScore × penalty
//   skipped step  → 0
//   unfinished    → bestScore × 0.5 × penalty (half credit for partial work)
//   lesson score  → mean over steps

import { grade } from "./graders.js";
import { checkpointForStep } from "./checkpoints.js";
import { HINT_RUNGS } from "./spec.js";
...
export function createSession(lesson) {
  return {
    ...
    stepIndex: 0,
    status: "active",
    steps: lesson.steps.map(freshStepState),
  };
}
...
export function submitStep(session, lesson, toolState) {
  ...
  if (result.pass) {
    ...
    stepState.score = result.score * hintPenalty(stepState.hintsUsed);
  }
...
export function lessonScore(session, lesson) {
  const steps = session.steps.map((s) => {
    ...
    else credit = s.bestScore * 0.5 * hintPenalty(s.hintsUsed);
    return {
      stepId: s.stepId,
      status: s.status,
      credit,
...
export function deserializeSession(json, lesson) {
  ...
  const steps = lesson.steps.map((def) => {
