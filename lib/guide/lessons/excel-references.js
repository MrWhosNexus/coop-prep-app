...
export const lesson = createLesson({
  ...
  moduleId: "excel",
  mode: "guided",
  // Outcome variant: the break-then-pin arc posed as requirements ("copy it
  // and observe", "make the whole column compute") without printing any
  // formula. The base graders' patterns pin reference SHAPES (J2/J8, the $
  // pin) — that shape IS this lesson's outcome, not a method detail: "the
  // copy translated" and "the total held still" are facts about the
  // references themselves, so the patterns stay. Only the final step's
  // grader narrows to pure outcome plus computed-not-typed rows.
  modes: ["outcome"],
  voice: true,
  ...
  steps: [
    {
      ...
      modes: {
        outcome: {
          instruction:
            "The table holds approved counts by race with the grand total in J8. In K2, compute the " +
            "first group's share of ALL approvals — and write both references plain, with no pinning. " +
            "That choice is deliberate: the next step shows what it costs.",
          hints: [
            "A share is this group's count over the total — J2 against J8.",
            "Leave the $ off both references on purpose; the point arrives when the formula moves.",
          ],
          // Base grader kept: the exact unpinned J2/J8 shape IS this step's
          // outcome — the lesson is about the references, not a function.
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Copy K2 into K3 — copy/paste, not retyping — and leave whatever happens in place. Then " +
            "click into K3 and read its formula against K2's: which parts moved, which of the moves " +
            "you wanted, and which one just broke the cell?",
          hints: [
            "Every plain reference is secretly an offset from the formula's own cell — it travels with a copy.",
            "One reference was supposed to follow the row; the other slid onto an empty cell.",
          ],
          // Base grader kept: the translated J3/J9 shape and its error ARE the
          // observation this step exists to produce.
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Fix the column: every group's share of approvals in K2:K7, each row computed from its own " +
            "count, all six dividing by the same total — and the whole thing built by writing ONE cell " +
            "and filling it down, not six separate formulas.",
          hints: [
            "Half of the formula should follow the row; the other half must be made to stay put.",
            "The six shares should sum to 1, with the biggest group holding well over half.",
          ],
          // Outcome grader: right values AND computed per-row (a typed column
          // must fail), but no required reference shape — six hand-written
          // divisions technically satisfy it, which outcome mode accepts as
