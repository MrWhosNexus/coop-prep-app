...
export const lesson = createLesson({
  ...
  steps: [
    {
      // REMEDIATION (2026-08-26): the old text told the learner to click a
      // "Load CSV" toolbar button that does not exist (the toolbar has "Load
      // HMDA sample" — the clean file — and "Import CSV…", a local-file
      // picker). The checkpoint already pre-loaded the extract, so the step
      // auto-passed while describing an impossible action. The text now
      // matches what actually happens: the data is on screen, orient in it.
      id: "load-raw",
      title: "Meet the raw extract",
      instruction:
        "The raw extract (hmda-raw.csv) is already loaded at A1: 8 columns (applicant_id through " +
        "approved) and 104 data rows in rows 2-105 — four MORE rows than the 100 applicants the " +
        "file is supposed to describe. Finding those four is this lesson.",
      hints: [
        "The headers sit in row 1; the last data row is 105.",
        "A2 reads \"A0001\" — a clean start to the id sequence.",
        "A105 reads \"A0072\" — an id you will meet again.",
      ],
      target: { kind: "sheet-cell", ref: "A105" },
      spotlightLabel: "Inspect the loaded extract — 104 rows, 100 applicants",
      ...
      modes: {
        outcome: {
          instruction:
            "The raw extract is already on screen — all 104 rows, headers in row 1. Establish how " +
            "many rows it holds versus how many applicants it claims to describe.",
          hints: [
            "The data runs from row 2 to row 105.",
            "104 rows, 100 claimed applicants — the gap is the lesson.",
          ],
        },
      },
    },
