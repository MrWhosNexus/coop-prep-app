...
import govProfiling from "./gov-profiling.js";
import govQualityRules from "./gov-quality-rules.js";
import govCompleteness from "./gov-completeness.js";
import govDuplicates from "./gov-duplicates.js";
import govStandardize from "./gov-standardize.js";
import govReconcile from "./gov-reconcile.js";
...
export const LESSONS = [
  ...
  tableauDualAxis,
  tableauShelvesGuide,
  // aigovernance module — the hands-on counterpart the AI Governance lessons
  // never had. Placed LAST on purpose: every one of these grades COUNTIF, IF,
  // TRIM or LEN, all of which are taught in the excel block above, so putting
  // them earlier would violate the prerequisite order the test enforces.
  // They run on the deliberately dirty hmda-raw.csv, not the clean extract.
  govProfiling,
  govQualityRules,
  govCompleteness,
  govDuplicates,
...
export const CURRICULUM_MAP = {
  ...
  // gov-1 "The four pillars: transparency, accountability, fairness,
  // auditability" — you cannot claim any of the four about data you have not
  // profiled, so the labs that establish WHAT IS IN THE FILE come first.
  "gov-1": ["gov-profiling", "gov-quality-rules", "gov-completeness"],
  // gov-2 "Disparate impact: intent vs. outcome" — gov-standardize is the
  // payoff: the approval-rate gap computed on unstandardised categories is
  // wrong, which is disparate impact arriving through a data defect rather
  // than through anyone's intent.
  "gov-2": ["gov-duplicates", "gov-standardize"],
  // gov-3 "Your governance project as your COOP case study" — reconciliation
  // ends with a summary an analyst could actually send to someone.
  "gov-3": ["gov-reconcile"],
};
