...
/**
 * OUTCOME-only fill check for the lesson's outcome mode: every cell in
 * H2:H101 must be a FORMULA (typed regions go stale when data changes) whose
 * value matches the region the sheet's own J:K table gives for that row's ZIP
 * — but no particular lookup construction is demanded, because in outcome
 * mode any computed route to 100 correct regions is a pass.
 * @param {object} toolState the sheet workbook being graded
 * @returns {object} a partial GradeResult
 */
function regionOutcomeCheck(toolState) {
  const sheet = resolveSheet(toolState, "Applicants");
  const table = getRangeValues(sheet, `J2:K${TABLE_LAST_ROW}`);
  const regionOf = new Map(table.map(([zip, region]) => [String(zip), region]));
  const zips = getRangeValues(sheet, "D2:D101");
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (!cell) {
      report({ kind: "missing", path: ref, expected, actual: null, hint: `${ref} is empty — every applicant needs a region.` });
      continue;
    }
    if (!cell.isFormula) {
      report({ kind: "method", path: ref, expected: "a computed region", actual: String(cell.input), hint: `${ref} holds a typed value — compute the region from the table so it survives a data refresh.` });
      continue;
    }
    const value = cell.value;
    if (typeof value === "string" && value === expected) {
    ...
    const shown = isError(value) ? value.code : JSON.stringify(value ?? null);
    report({ kind: "wrong", path: ref, expected, actual: value, hint: `${ref} covers ZIP ${zip} — the table says ${expected}, but the cell shows ${shown}.` });
  }
  ...
  return {
    ...
    score: ok / 100,
    message: pass
      ? "All 100 regions computed and matching the table."
      : `${100 - ok} of 100 rows in column H aren't right yet — ${diff[0].hint}`,
    diff,
    expected: "H2:H101 computed from the linked table",
    actual: `${ok}/100 rows correct`,
  };
}
...
export const lesson = createLesson({
  ...
  moduleId: "excel",
  mode: "guided",
  // Outcome variant: the two-halves build posed as its underlying questions —
  // "where does this ZIP sit?", "what sits there?" — with both function names
  // withheld. The named-function graders are dropped: in outcome mode any
  // computed route to the position, the region, and the filled column is a
  // pass (including the modern lookup this lesson's scaffolded variant
  ...
  steps: [
    {
      ...
      modes: {
        outcome: {
          instruction:
            `The ZIP-to-region table sits in J2:K${TABLE_LAST_ROW}. Before pulling any region out of ` +
            "it, answer a narrower question in H2: at which position in the table's ZIP column does " +
            "the first applicant's ZIP (D2) sit? The answer is a row number within the table, not a " +
            "region.",
          hints: [
            "The question is \"which slot?\", so the right answer is a small whole number.",
            "D2 is 48217 — scan the table's ZIP column for exactly that value.",
          ],
          grader: { type: "cellFormula", ref: "H2", expectedValue: 7 },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "A position is only useful if you cash it in. Rework H2 so it shows the first applicant's " +
            "actual REGION — the entry sitting at that position in the table's region column.",
          hints: [
            "You know where in the table the ZIP sits; fetch what the region column holds at that spot.",
            "D2's ZIP is Detroit's, so the answer is the region Michigan belongs to.",
          ],
          grader: { type: "cellFormula", ref: "H2", expectedValue: "Midwest" },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Every applicant needs a region, not just the first: H2:H101, all computed from the table. " +
            "Spot-check the ends — H3 (an El Paso ZIP) should say South, H101 (a Los Angeles ZIP) " +
            "should say West.",
          hints: [
            "If regions break part-way down the column, something in your formula moved that shouldn't have.",
            "The parts that point at the table must hold still; the part that points at the row must move.",
          ],
          grader: { type: "predicate", fn: regionOutcomeCheck, label: "Column H computed and correct for all 100 rows" },
        },
      },
    },
  ],
