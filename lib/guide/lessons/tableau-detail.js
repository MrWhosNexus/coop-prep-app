...
// Outcome-mode fixture: the whole-file anchor point. Derived by summing the
// per-race figures above: total income 7,147,500 and total loans 21,735,500
// over 100 rows (the per-gender table cross-checks to the same totals).
export const WHOLE_FILE_POINT = [{ "AVG(income)": 71475, "AVG(loan_amount)": 217355 }];

// Outcome-mode fixture: the 14 race × gender combinations that actually occur
// in hmda-sample.csv (out of a possible 6 × 3 = 18). Groups only, no
// measures: the grade is "exactly these marks exist".
export const RACE_GENDER_COMBOS = [
  ["American Indian", "Female"], ["American Indian", "Male"],
...
].map(([race, gender]) => ({ race, gender }));

/**
 * OUTCOME predicate for the variant mode: the axes must stay two continuous
 * measures — no discrete pill on Rows or Columns. This is the observable
 * "the grouping costs no axis" constraint, without pinning WHICH axis-free
 * shelf (Detail or Color both slice for free, and both are correct routes).
 */
function axesStayContinuous(toolState) {
  const spec = toolState?.spec;
  if (!spec) return { pass: false, message: "No view yet — the check needs a spec.", diff: [] };
  const discreteAxes = [...(spec.rows ?? []), ...(spec.columns ?? [])].filter((e) => e?.discrete);
  if (discreteAxes.length === 0) {
    return { pass: true, message: "Both axes stay continuous — the grouping rides along without headers." };
  }
  const names = discreteAxes.map((e) => e.field).join(", ");
  const hint = `${names} sits on an axis and creates headers — the slicing must not cost an axis. Use a Marks-card shelf instead.`;
  return {
    pass: false,
    message: hint,
    expected: "no discrete pills on Rows or Columns",
    actual: names,
    diff: [{ kind: "wrong", path: "rows/columns", expected: "continuous pills only", actual: names, hint }],
  };
}
...
export const lesson = createLesson({
  ...
  moduleId: "tableau",
  mode: "guided",
  // Outcome variant: same ids and checkpoints; questions state what the view
  // must show, graders read the computed marks and the axes' continuity.
  modes: ["outcome"],
  voice: true,
  ...
  steps: [
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Where does the whole portfolio sit? Plot typical income against typical loan size for " +
            "the file as ONE point — the anchor the group comparison will need.",
          hints: [
            "Both axes carry a value, not categories — that's what makes it a scatter.",
            "With no grouping anywhere, the view collapses to a single point near (71.5k, 217.4k).",
          ],
          // Shelf recipe dropped; the mark check reads resolveMark() (the
          // rendered mark) and vizData pins the single point's coordinates.
          grader: {
            type: "allOf",
            of: [
              { type: "vizSpec", expected: { mark: "scatter" } },
              { type: "vizData", expected: WHOLE_FILE_POINT, keyFields: [], tolerance: 1 },
            ],
          },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Which race groups sit where? Explode the single point into one per race group — " +
            "without giving the grouping an axis of its own.",
          hints: [
            "The axes stay income vs loan; the grouping rides along without headers.",
            "Six points; among the big groups Black sits lowest on both axes (66.6k, 197k).",
          ],
          // The Detail-shelf predicate is dropped ON PURPOSE: Color also
          // slices axis-free and is a correct route. The outcome graded is
          // "six points at the true coordinates, axes still continuous".
          grader: {
            type: "allOf",
            of: [
              { type: "predicate", fn: axesStayContinuous, label: "Axis-free slicing" },
              { type: "vizData", expected: SCATTER_POINTS_BY_RACE, keyFields: ["race"], tolerance: 1 },
            ],
          },
        },
    ...
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Now split each race's point by gender, still keeping every grouping off the axes. How " +
            "many marks appear — and why isn't it 18?",
          hints: [
            "Only combinations that exist in the data get a mark: 14 of them.",
            "Race and gender both ride on the Marks card; the axes stay income vs loan.",
          ],
          // Method grader (full vizSpec shelf map) swapped for OUTCOME: the
          // 14 real race x gender marks must exist (and no phantom 18), with
          // the axes still continuous. Which Marks-card shelf carries which
          // field is deliberately not pinned.
          grader: {
            type: "allOf",
            of: [
              { type: "predicate", fn: axesStayContinuous, label: "Axis-free slicing" },
              { type: "vizData", expected: RACE_GENDER_COMBOS, keyFields: ["race", "gender"] },
            ],
          },
        },
      },
