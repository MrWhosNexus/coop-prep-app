...
// Outcome-mode fixture: the six race groups with no measure attached — grades
// "one mark per group exists" regardless of which shelf produced the slicing.
const RACE_GROUPS = EXPECTED_RATES.map(({ race }) => ({ race }));
...
export const lesson = createLesson({
  ...
  moduleId: "tableau",
  mode: "guided",
  // The outcome variant re-poses every step as the business question with the
  // mechanics withheld: same step ids, same checkpoints, and outcome graders
  // (vizData) wherever one exists — any route that produces the right chart
  // passes. See excel-pivot.js for the pattern this follows.
  modes: ["outcome"],
  voice: true,
  ...
  steps: [
    {
      ...
      modes: {
        outcome: {
          instruction:
            "The data records each decision as APPROVED or DENIED, but a rate needs numbers. Add a " +
            "field named exactly is_approved that reads 1 for an approved application and 0 for a " +
            "denied one — the rest of the lesson builds on that name.",
          hints: [
            "The share of a group that was approved is just the average of a flag that is 1 for approved, 0 for denied.",
            "Done right, the new field marks exactly 76 of the 100 applications with a 1.",
          ],
          // No grader override: the base predicate is already an OUTCOME
          // grader — any expression flagging the 76 approvals passes.
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Set the view up to compare the six race groups side by side — one slot per group, no numbers yet.",
          hints: [
            "The field that distinguishes the groups is race.",
            "You should see the six groups, American Indian through White, and nothing else.",
          ],
          // Method grader (vizSpec: race on Columns) swapped for OUTCOME:
          // the view must emit one mark per race group, wherever the pill sits.
    ...
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "What share of each race group was approved? Make the view answer with one value per group.",
          hints: [
            "A group's approval share is its approved count relative to its size — the 1/0 flag gets you there in one move.",
            "White should land at 0.86 and Black at 0.5625.",
          ],
          // Method grader (vizSpec pill recipe) swapped for OUTCOME: the view
          // must compute the six true rates, by any route.
          grader: { type: "vizData", expected: EXPECTED_RATES, keyFields: ["race"] },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Present the rates as a bar chart a reviewer can read in five seconds — one bar per race " +
            "group, bar height = approval rate. What story does it tell?",
          hints: [
            "Bars need the groups on one axis and the rate on the other.",
            "The Black bar should sit at 0.5625 and White at 0.86 — a 30-point gap.",
          ],
          // The shelf recipe is dropped; the mark check stays because it reads
          // resolveMark() — the RENDERED mark, an outcome — not the shelves.
          grader: {
            type: "allOf",
            of: [
