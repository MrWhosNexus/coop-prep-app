...
/**
 * Outcome grader for the years-with-decimal step. This exists because the
 * override below used to carry NO grader, so outcome mode silently fell back
 * to the base cellFormula grader — whose pattern demands (B2-A2)/365.25
 * literally and whose mustNotUse bans DATEDIF outright. Under an outcome
 * instruction that only says "express the span as a fraction of an
 * average-length year", that marked =C2/365.25 (dividing the days column the
 * learner just built — the natural move) and =DATEDIF(A2,B2,"D")/365.25 as
 * WRONG: the exact reject-a-correct-answer trap Phase 1 removed, re-created
 * by an inherited grader.
 *
 * What it still must refuse: the DATEDIF("Y")+MOD hybrid the file header
 * documents. On this 32-day row the hybrid computes the SAME value as the
 * correct division, so a value-only check cannot tell them apart — the
 * 365.25 divisor requirement and the "Y"-unit ban are the minimum method
 * constraints that keep it out, while any route to the day count (B2-A2,
 * C2, DATEDIF "D") passes.
 * @param {object} toolState
 * @returns {object} partial GradeResult
 */
function outcomeYearsD2(toolState) {
  const sheet = resolveSheet(toolState, "Data");
  const cell = getCell(sheet, "D2");
  const fail = (hint, kind = "method") => ({
    pass: false,
    message: hint,
    diff: [{ kind, path: "D2", expected: "a day span divided by 365.25", actual: cell ? String(cell.input) : null, hint }],
  });

  if (!cell || cell.input === "" || cell.input == null) {
    return fail("D2 is empty — express the first span as a fraction of a year.", "missing");
  }
  if (!cell.isFormula) {
    return fail("D2 holds a typed value instead of a formula — compute the fraction from the dates.");
  }
  const input = String(cell.input).toUpperCase();
  if (input.includes('"Y"')) {
    return fail('Whole calendar years plus a remainder double-counts — divide the raw day span by 365.25 instead of using the "Y" unit.');
  }
  if (!input.includes("365.25")) {
    return fail("Divide the day span by 365.25, the average length of a year.");
  }
  const got = getValue(sheet, "D2");
  const want = YEARS_BETWEEN[0][0];
  if (typeof got !== "number" || Math.abs(got - want) > 1e-6) {
    return {
      pass: false,
      message: `D2 shows ${got}, but 32 days is about ${want.toFixed(4)} years.`,
      diff: [{ kind: "wrong", path: "D2", expected: want, actual: got, hint: "The first span is 32 days — its fraction of a year starts 0.08…" }],
    };
  }
  return { pass: true, message: "32 days expressed as a fraction of an average year." };
}
...
export const lesson = createLesson({
  ...
  moduleId: "excel",
  mode: "guided",
  // Outcome variant: the processing-time questions posed without handing over
  // either the legacy function or the subtraction. The fill steps grade pure
  // OUTCOME (rangeValues) — and the 730-day row still does its sentinel job
  // there, because the broken whole-years-plus-remainder hybrid this lesson's
  // header describes diverges from the verified value on exactly that row.
  modes: ["outcome"],
  voice: true,
  ...
  steps: [
    {
      ...
      modes: {
        outcome: {
          instruction:
            "How long did the lender sit on the first application? Columns A and B hold when it was " +
            "submitted and when it was approved — put the processing time, in days, in C2, computed " +
            "from those two cells.",
          hints: [
            "Dates in a sheet are day numbers underneath, which makes 'how far apart' simple arithmetic.",
            "The first application took just over a month.",
          ],
          // Computed, by any route — the scaffolded step's whole point is that
          // two different methods are equally right here.
          grader: { type: "cellFormula", ref: "C2", expectedValue: 32 },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          // Pure fill-down with no decision in it: the outcome re-pose folds
          // it into the previous question — "now answer it for every
          // application" — instead of posing a trivial standalone task.
          instruction:
            "Now answer the same question for every application: C2:C7 should show each one's " +
            "processing time in days. One of them is an outlier — it sat for two full years.",
          hints: [
            "Each row's answer comes from that row's own pair of dates.",
            "Five applications took 30-70 days; the last took 730.",
          ],
          grader: { type: "rangeValues", range: "C2:C7", expected: DAYS_BETWEEN },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Days are exact but hard to compare at a glance; leadership wants the processing time in " +
            "years, with decimals. In D2, express the first application's span as a fraction of an " +
            "average-length year.",
          hints: [
            "An average year, leap days included, runs 365.25 days.",
            "32 days is a small fraction of a year — the answer starts 0.08…",
          ],
          // An EXPLICIT outcome grader, not the inherited base one. Leaving
          // this field off looked like "method grader kept, deliberately",
          // but what it actually inherited was the base pattern (B2-A2)/365.25
          // plus a blanket DATEDIF ban — which failed =C2/365.25 and
          // =DATEDIF(A2,B2,"D")/365.25, both CORRECT under this instruction.
          // outcomeYearsD2 keeps only the constraint the hybrid bug needs
          // (the 365.25 divisor, no "Y" unit) and accepts every day-count
          // route. test/outcome-modes-excel.test.js now flags any outcome
          // override that inherits a method-locked grader, so this cannot
          // quietly regress.
          grader: {
            type: "predicate",
            fn: outcomeYearsD2,
            label: "The span expressed as days divided by 365.25, by any day-count route",
          },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          // Fill-down folded into the previous question, as with fill-days —
          // but here the last row is the real check: it is the one span where
          // a wrong year definition shows up as a full-year error.
          instruction:
            "Extend the years column to every application, D2:D7 — and look hard at the last one. " +
            "A 730-day span should read just under two years; if yours says almost three, the year " +
            "arithmetic is double-counting somewhere.",
          hints: [
            "Each row converts its own day span with the same 365.25 divisor.",
            "D7 should show about 1.9986, not 2.9986.",
          ],
          grader: { type: "rangeValues", range: "D2:D7", expected: YEARS_BETWEEN, tolerance: 1e-6 },
        },
      },
    },
  ],
