...
export const lesson = createLesson({
  ...
  moduleId: "excel",
  mode: "guided",
  // REFERENCE IMPLEMENTATION for lesson modes (see spec.js LESSON_MODES).
  // "outcome" re-poses each step as the business question with the mechanics
  // withheld: same checkpoints, same step ids, outcome-only graders
  // (pivotResult — ANY spec that computes the right table passes; the
  // scaffolded variant's pivotSpec method checks are dropped on purpose).
  modes: ["outcome"],
  voice: true,
  ...
  steps: [
    {
      ...
      instruction:
        "Load the HMDA extract into the sheet — the toolbar has a \"Load HMDA sample\" button for exactly this file. You should end up with " +
        "7 columns (applicant_id through approved) and 100 applicants in rows 2-101.",
      hints: [
        "The sheet toolbar has a \"Load HMDA sample\" button — that is this dataset.",
        ...
        "Press \"Load HMDA sample\" in the sheet toolbar — it loads this file at A1 for you. Cell G1 should read \"approved\" and A101 should read \"A0100\".",
      ],
      ...
      modes: {
        outcome: {
          instruction:
            "Get the HMDA extract (/data/hmda-sample.csv) into the sheet so all 100 applicants are ready to analyze.",
          hints: [
            "The dataset lives at /data/hmda-sample.csv.",
            "Headers belong in row 1 — load with A1 as the origin.",
          ],
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "How many applicants are there in each race group? Produce a table that answers the question.",
          hints: [
            "You need one row per race group, each with a count of applicants.",
            "A pivot table over A1:G101 gets you there fastest.",
          ],
          grader: { type: "pivotResult", expected: { rows: ["race"], values: [COUNT_VALUE] } },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "For each race group, how many applications were APPROVED and how many were DENIED? Extend your table to show both outcomes per group.",
          hints: [
            "The table needs a second grouping: outcome across the page.",
            "Black should come out 9 approved / 7 denied; White 43 / 7.",
          ],
          grader: {
            type: "pivotResult",
            expected: { rows: ["race"], cols: ["approved"], values: [COUNT_VALUE] },
    ...
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "What SHARE of each race group was approved? Counts can't be compared across groups of different sizes — express each group's outcomes as percentages of that group.",
          hints: [
            "Each race's APPROVED and DENIED cells should sum to 100% of that group.",
            "Done right, Black reads 56.25% approved and White 86%.",
          ],
