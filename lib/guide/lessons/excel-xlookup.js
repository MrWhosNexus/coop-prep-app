...
export const lesson = createLesson({
  ...
  moduleId: "excel",
  mode: "guided",
  // Outcome variant: the enrichment task posed as what must be TRUE ("every
  // applicant carries the region the table assigns their ZIP") with the
  // lookup function withheld. Named-function graders are dropped — the
  // regionFillCheck predicate is already pure outcome (it derives every
  // expected region from the sheet itself), so it carries the fill step
  // alone.
  modes: ["outcome"],
  voice: true,
  ...
  steps: [
    {
      ...
      modes: {
        outcome: {
          // Grader unchanged — cellValue is already the outcome check.
          instruction:
            "A lookup table with a hole in it corrupts every row that hits the hole. On the Regions " +
            "tab, one ZIP is missing its census region: 10001, New York, NY. Complete the table before " +
            "you build anything on top of it.",
          hints: [
            "The US census regions are Northeast, Midwest, South and West.",
            "The neighboring rows — Massachusetts and New Jersey ZIPs — answer it for you.",
          ],
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Back on the Applicants tab: each applicant's H cell should carry the region the table in " +
            "J2:K23 assigns their ZIP (column D). Start with H2 — computed from the table, and built so " +
            "the same cell can later be applied to the whole column without breaking.",
          hints: [
            "The answer for H2 comes from finding D2's ZIP in the table's left column and returning its neighbor.",
            "Think ahead: which parts of the formula must hold still when it moves down 99 rows?",
          ],
          grader: { type: "cellFormula", sheet: "Applicants", ref: "H2", expectedValue: "Midwest" },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Enrich the whole file: all 100 applicants in H2:H101 carrying the region their ZIP maps " +
            "to, with no errors anywhere in the column. Spot-check the far end — H101 is a Los Angeles " +
            "ZIP.",
          hints: [
            "If the column breaks part-way down, something that pointed at the table moved as the formula did.",
            "H101 should read West.",
          ],
          // Outcome grader: regionFillCheck alone — it derives every expected
          // region from the sheet's own table, and the H101 cellFormula it
          // rode with demanded the function by name, which the mode withholds.
          grader: {
            type: "allOf",
            of: [
              { type: "predicate", fn: regionFillCheck, label: "Column H matches the Regions table" },
              { type: "cellFormula", sheet: "Applicants", ref: "H101", expectedValue: "West" },
            ],
          },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "A new applicant, A0101, just landed in row 102 with ZIP 99999 — a ZIP the Regions table " +
            "has never heard of. H102 must read Unknown instead of an error, while still resolving " +
            "correctly for any ZIP the table DOES know.",
          hints: [
            "The cell needs a graceful answer for the no-match case, not a patched-over error.",
            "The fallback is text: Unknown.",
          ],
          // Outcome grader: computed + the graceful value; no function named,
          // so a guarded lookup of any construction passes.
          grader: { type: "cellFormula", sheet: "Applicants", ref: "H102", expectedValue: "Unknown" },
        },
      },
    },
  ],
