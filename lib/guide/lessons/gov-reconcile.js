...
export const lesson = createLesson({
  ...
  steps: [
    {
      id: "load-both",
      title: "Load both systems side by side",
      instruction:
        "Load /data/hmda-raw.csv at A1 and /data/hmda-servicing.csv at K1, so the two systems sit side " +
        "by side: origination in A:H (rows 2-105), servicing in K:N (rows 2-100). Note the row counts " +
        "disagree before you compute anything — 104 vs 99 — and the whole lesson is explaining that gap.",
      hints: [
        "Load CSV twice: hmda-raw.csv with origin A1, then hmda-servicing.csv with origin K1.",
        "Servicing has 4 columns (applicant_id, zip_code, loan_amount, servicer), so it occupies K:N.",
        "After both loads, H1 reads \"approved\" and N1 reads \"servicer\".",
        "The last servicing row is K100 = \"B903\" — an id the raw file has never heard of. Remember it.",
      ],
      target: { kind: "sheet-cell", ref: "K1" },
      spotlightLabel: "Raw at A1, servicing at K1",
      ...
      modes: {
        outcome: {
          instruction:
            "Get both systems into one sheet: the raw extract at A1, the servicing extract at K1.",
          hints: [
            "Two Load CSV operations, two origins: A1 and K1.",
            "When done, H1 reads \"approved\" and N1 reads \"servicer\".",
          ],
        },
      },
    },
