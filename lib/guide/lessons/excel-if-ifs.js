...
/**
 * OUTCOME-only variant of bandFillCheck for the lesson's outcome mode: every
 * cell in O2:O101 must be a FORMULA (typed bands can't defend themselves when
 * a loan amount changes) whose value is the band column E demands — but no
 * particular function is required, because in outcome mode a nested-IF ladder
 * or any other computed route to the right 100 bands is a pass.
 * @param {object} toolState the sheet workbook being graded
 * @returns {object} a partial GradeResult
 */
function bandOutcomeCheck(toolState) {
  const sheet = resolveSheet(toolState);
  const loans = getRangeValues(sheet, "E2:E101");
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const cell = getCell(sheet, ref);
    if (!cell) {
      report({ kind: "missing", path: ref, expected, actual: null, hint: `${ref} is empty — every loan needs a band.` });
      continue;
    }
    if (!cell.isFormula) {
      report({ kind: "method", path: ref, expected: "a computed band", actual: String(cell.input), hint: `${ref} holds a typed band — compute it from the loan amount so it updates when the data does.` });
      continue;
    }
    const value = cell.value;
    if (typeof value === "string" && value.trim() === expected) {
  ...
  return {
    ...
    score: ok / 100,
    message: pass
      ? "All 100 loans are banded, computed from their amounts."
      : `${100 - ok} of 100 loans in column O aren't banded right yet — ${diff[0].hint}`,
    diff,
    expected: "O2:O101 banded, computed from the loan amounts in E",
    actual: `${ok}/100 rows correct`,
  };
}
...
export const lesson = createLesson({
  ...
  moduleId: "excel",
  mode: "guided",
  // Outcome variant: the verdicts and bands posed as the decisions a reviewer
  // needs, with the branching functions withheld. Graders drop the named-
  // function requirements: any computed route to the right verdicts/bands is
  // a pass (cellFormula with only expectedValue, rangeValues, and the
  // outcome-only band predicate above).
  modes: ["outcome"],
  voice: true,
  ...
  steps: [
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Does the first group clear the four-fifths line? Its ratio sits in M2; the legal threshold " +
            "is 0.8. N2 should read FAIL or PASS — computed from the ratio, so the verdict flips itself " +
            "if the data changes.",
          hints: [
            "The verdict is a decision the sheet makes, not a word you type: under the line fails.",
            "American Indian's ratio is 0.775 — which side of 0.8 is that?",
          ],
          grader: { type: "cellFormula", ref: "N2", expectedValue: "FAIL" },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Which of the six groups clear the four-fifths line, and which don't? N2:N7 should hold " +
            "every group's verdict against its own ratio. (Two of the failing groups have only three " +
            "applicants each — the four-fifths calculator lesson's verdict column shows how a careful analyst " +
            "reports those.)",
          hints: [
            "Each row judges its own ratio in column M against the same 0.8 line.",
            "Only two groups should come out on the passing side.",
          ],
          grader: { type: "rangeValues", range: "N2:N7", expected: FLAGS },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "The governance dashboard buckets every loan into three dollar bands: \"Under $150k\", " +
            "\"$150k-$250k\", and \"Over $250k\". In O2, compute which band the first applicant's loan " +
            "(E2, a $171,000 loan) belongs to.",
          hints: [
            "Three bands means the cell has to pick between more than two outcomes.",
            "The band labels must match the dashboard's exactly, quotes and all.",
          ],
          grader: { type: "cellFormula", ref: "O2", expectedValue: "$150k-$250k" },
        },
      },
    },
    {
      ...
      modes: {
        outcome: {
          instruction:
            "Band the whole file: every applicant in O2:O101 gets a computed band. Sanity-check the " +
            "extremes — a $69,500 loan (row 6) belongs in the bottom band and a $306,000 loan (row 12) " +
            "in the top one.",
          hints: [
            "If a small loan lands in the middle band, the order you check the cutoffs in is wrong.",
            "Every cell in O should still be computed from its own row's loan amount.",
          ],
          grader: {
            type: "allOf",
            of: [
              { type: "cellValue", ref: "O6", expected: "Under $150k" },
              { type: "cellValue", ref: "O12", expected: "Over $250k" },
              { type: "predicate", fn: bandOutcomeCheck, label: "All 100 loans banded, computed from their amounts" },
            ],
          },
        },
      },
